...
## Mandatory Cases for Every Endpoint

Cover these four cases for every endpoint — this is not optional:
...
For `AllowAny` endpoints (login), replace cases 1 and 2 with:
...
Always use `force_authenticate` — never real Knox tokens:
...
# Database state — always verify for write operations
...
## DB Verification for Write Endpoints
...
For service-layer operations that update status, verify the subtype model directly:
...
After writing failing tests, stop and present them to the user for explicit approval before writing any implementation code.
